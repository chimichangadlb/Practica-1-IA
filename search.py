...
class CornerReversiState:
    ...
    def legalMoves(self):
        ...
        return get_valid_moves(self.board, self.height, self.width, self.player1, self.player2, self.blocked_cell_label, self.ignore_block_cells_in_captures) # RETURN THE LIST OF VALID MOVES
    ...
    def result(self, move):
        ...
        return CornerReversiState(board=result_board, cur_player=adversary, player1=self.player1, player2=self.player2, ignore_block_cells_in_captures=self.ignore_block_cells_in_captures, height=self.height, width=self.width) # RETURN THE NEW STATE CONSIDERING THE UPDATES
...
def build_game_tree(search_problem, max_depth):
    ...
    def expand(state, depth):
        ...
        sucessors = search_problem.getSuccessors(state)
        ...
        stats["branching_sum"] += len(sucessors)
